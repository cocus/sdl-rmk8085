...
SDL_CreateTerminal (void)
{
    ...
    /* Scaling */
    terminal->scale.width  = 1.0;
    terminal->scale.height = 1.0;
    ...
    SDL_GetEventFilter(&terminal->event_filter, NULL);
...
int SDL_TerminalEventFilter (void* userdata, SDL_Event *event)
{
    ...
    if (SDL_ActiveTerminal) {
        ...
        if ((SDL_ActiveTerminal->event_filter != NULL) && (!status))
            return (SDL_ActiveTerminal->event_filter)(NULL, event);
    }
    return status;
}
...
{
    if (!terminal->visible || terminal->disable_kb_input)
        return 1;
    ...
    case SDL_MOUSEMOTION:
        if ((event->motion.x > terminal->position.x) &&
            ...
            (event->motion.x < (terminal->position.x+terminal->psize.w*terminal->scale.width)) &&
            (event->motion.y < (terminal->position.y+terminal->psize.h*terminal->scale.height))) {
            terminal->active = 1;
        } else {
            terminal->active = 0;
        }
 ...
 */
int
SDL_TerminalBlit (SDL_Terminal *terminal, SDL_Renderer *renderer)
{
    SDL_TerminalRenderCursor (terminal);
...
#endif
    //if (!(screen->flags & SDL_OPENGL)) {
        SDL_SetSurfaceAlphaMod (terminal->surface, 255);
        float scale_width = terminal->scale.width;
        float scale_height = terminal->scale.height;
        SDL_Rect src = {0,0, terminal->surface->w, terminal->surface->h};
        SDL_Rect dst = {terminal->position.x * scale_width, terminal->position.y * scale_height, terminal->surface->w * scale_width, terminal->surface->h * scale_height};
        SDL_Texture *texture = SDL_CreateTextureFromSurface(renderer, terminal->surface);
        int ret = SDL_RenderCopy(renderer, texture, &src, &dst);
...
SDL_TerminalClear (SDL_Terminal *terminal)
{
    ...
    /* Inner background */
    color = SDL_MapRGBA (fmt, terminal->bg_color.r, terminal->bg_color.g, terminal->bg_color.b, terminal->bg_color.a);
...
int
SDL_TerminalDisableKB (SDL_Terminal *terminal)
{
    terminal->disable_kb_input = 1;
    return 0;
}

int
SDL_TerminalEnableKB (SDL_Terminal *terminal)
{
    terminal->disable_kb_input = 0;
    return 0;
}
...
int
SDL_TerminalSetScaling (SDL_Terminal *terminal, float scale_width, float scale_height)
{
    if (!terminal)
    {
        return -1;
    ...
    terminal->scale.width = scale_width;
    terminal->scale.height = scale_height;
...
{
    if (c == '\n') {
        SDL_TerminalEraseChar (terminal,
...
{
    Uint32 color = SDL_MapRGBA (terminal->surface->format, terminal->bg_color.r, terminal->bg_color.g, terminal->bg_color.b, terminal->bg_color.a);
    SDL_SetSurfaceAlphaMod(terminal->surface, 255);
