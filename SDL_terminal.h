...
/*
 * Useful structure to hold a couple of floats
 */
typedef struct Vec2f {
    union {float x; float w; float width;  float row;};
    union {float y; float h; float height; float column;};
} Vec2f;
...
typedef struct SDL_Terminal {
    ...
    int             disable_kb_input;       /* Terminal disable kb_input processing */
    ...
    Vec2f           scale;                  /* scale (width, height) */
...
int SDL_TerminalBlit (SDL_Terminal *terminal, SDL_Renderer *renderer);
...
int SDL_TerminalDisableKB (SDL_Terminal *terminal);
